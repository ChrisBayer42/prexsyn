...
def download(local: str | pathlib.Path) -> None:
    remote = "https://raw.githubusercontent.com/mirunacrt/synflownet/main/src/synflownet/models/cache/bengio2021flow_proxy.pkl.gz"
...
def mpnn_feat(mol, ifcoord=True, panda_fmt=False, one_hot_atom=False, donor_features=False):
    ...
    for i, atom in enumerate(mol.GetAtoms()):
        ...
        if one_hot_atom:
            atmfeat[i, min(ntypes + 9 + atom.GetAtomicNum() - 1, nfeat - 1)] = 1
        else:
...
def coalesce(edge_index, edge_attr, m, n):
    ...
    mask[1:] = idx[perm][1:] != idx[perm][:-1]

    # sum up weights of duplicate edges
    ...
    for i in range(mask.shape[0]):
        if mask[i]:
            ...
            new_edge_attr[-1] += edge_attr[i]
    new_edge_attr = np.stack(new_edge_attr, axis=0)
...
def mol_to_graph_backend(atmfeat, coord, bond, bondfeat, props={}, data_cls=Data):
    ...
    if bond.shape[0] > 0:
        edge_index = np.concatenate([bond.T, np.flipud(bond.T)], axis=1)
        edge_attr = np.concatenate([bondfeat, bondfeat], axis=0)
...
class sEH_proxy:
    def __init__(self) -> None:
        oracles_dir = pathlib.Path("./data/oracles")
        oracles_dir.mkdir(parents=True, exist_ok=True)
        oracle_path = oracles_dir / "bengio2021flow_proxy.pkl.gz"
        if not oracle_path.exists():
            download(oracle_path)
        self.model = load_original_model(cache=True, location=oracle_path).cuda()
        self.model.eval()
    ...
    def __call__(self, mol: Chem.Mol | list[Chem.Mol]) -> float | list[float]:
        ...
        if isinstance(mol, Chem.Mol):
            mol = [mol]
            single = True
        graphs = [mol2graph(m) for m in mol]
        batch = mols2batch(graphs).cuda()
        with torch.no_grad():
            out = self.model(batch).squeeze(-1).cpu().numpy()
        out = out / 8.0  # According to Appendix A.1 of Cretu et al. (2024) SynFlowNet.
        if single:
            return float(out[0])
        return out.tolist()
